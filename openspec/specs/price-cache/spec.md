...
File-based JSON cache per data source storing normalized price records. Refreshes weekly on Monday for fuel-prices.eu data and on its own schedule for SSB. Serves stale data as fallback when upstream is unreachable.
