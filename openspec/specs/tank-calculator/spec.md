# tank-calculator Specification

## Purpose

Tank fill cost comparison widget. Multiplies current API fuel prices (Euro 95 and Diesel) by a user-adjustable tank size (1-200L, default 50L) to show what a full tank costs in SEK and in the selected country's native currency, plus a savings delta between the two fuels. Reacts to `CountryStateService` country changes and re-fetches prices on selection.
